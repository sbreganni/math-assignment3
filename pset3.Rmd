...
```{=latex}
\begin{answerbox} The dimensionality of this covariance is m x m, since the covariance matrix pairs the covariance of each column against the     other, and since there are 200 columns (and 150 rows), then we end up with a 200 x 200 matrix. 
\end{answerbox}
```
...
`{{=latex}}  \begin{answerbox}  As can be seen above, it takes around 10 principal components to have a reasonable degree of confidence that we are indeed looking at a cat. \end{answerbox}`
